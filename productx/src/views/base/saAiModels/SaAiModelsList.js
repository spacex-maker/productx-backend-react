...
const SaAiModelsList = ({ fixedModelType, excludeModelType }) => {
  ...
  useEffect(() => {
    fetchData();
  }, [currentPage, pageSize, searchParams, fixedModelType, excludeModelType]);
    ...
    try {
      ...
      const response = await api.get('/manage/sa-ai-models/list', {
        params: {
          ...
          pageSize,
          ...filteredParams,
          ...(fixedModelType ? { modelType: fixedModelType } : {}),
          ...(excludeModelType ? { excludeModelType } : {}),
        },
      });
